...
func TestIngress(t *testing.T) {
	...
	// Wait until the Ingress is reconciled with the load balancer status
	...
	// now our ingress is in the expected state assert DNS and Transforms correct
	ingress = GetIngress(test, namespace, name)
	...
	test.Eventually(Ingress(test, namespace, name)).WithTimeout(TestTimeoutShort).Should(And(
		Satisfy(OriginalSpecUnchanged(test, &originalIngress.Spec)),
		Satisfy(TransformedSpec(test, GetDefaultSpec(glbcHost, secretName))),
	...
	withCustomDomain := GetDefaultSpec(glbcHost, secretName)
	//bit nasty but do it this way as this is how it is appended in the reconcile and as we check just the value of the spec diff matches they have to be in the same order
