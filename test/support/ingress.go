...
func TransformedSpec(test Test, expectedSpec *networkingv1.IngressSpec) func(ingress *traffic.Ingress) bool {
	return func(ingress *traffic.Ingress) bool {
		...
		if !equality.Semantic.DeepEqual(expectedSpec.Rules, ingress.Spec.Rules) {
			expectedPatches = append(expectedPatches, patch{
				OP:    "replace",
				Path:  "/rules",
				Value: expectedSpec.Rules,
			})
		}
		if !equality.Semantic.DeepEqual(expectedSpec.TLS, ingress.Spec.TLS) {
			expectedPatches = append(expectedPatches, patch{
				OP:    "replace",
				Path:  "/tls",
				Value: expectedSpec.TLS,
			})
		}
		for _, st := range ingress.GetSyncTargets() {
			// check the patches for each cluster they should all look the same
			clusterPatch := ingress.Annotations[workload.ClusterSpecDiffAnnotationPrefix+st]
			p, err := json.Marshal(expectedPatches)
			test.Expect(err).NotTo(gomega.HaveOccurred())
			if string(p) != clusterPatch {
				//test.T().Log("expected changes are not in the applied patches", string(p), clusterPatch)
				return false
			}
		}
		return true
...
func GetDefaultSpec(host, tlsSecretName string) *networkingv1.IngressSpec {
